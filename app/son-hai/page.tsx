...
import { Volume2, VolumeX, ChevronLeft, Mountain, Waves, Feather, Map, Compass, Eye } from "lucide-react";
...
export default function SonHaiPage() {
  ...
  // Hiệu ứng Parallax cho đại ngàn viễn cổ
  ...
  const mythicalCreatures = [
    { name: "Giao Long", title: "Bá Chủ Thủy Vực", desc: "Ngự trị nơi vực nước sâu cuộn sóng bạt ngàn, mang trong mình tà lực điều khiển dông bão, sấm chớp và những đợt thủy triều nhấn chìm lục địa.", icon: <Waves size={22} />, color: "from-[#00BFFF]/20 to-transparent", glow: "shadow-[0_0_30px_rgba(0,191,255,0.2)]" },
    { name: "Cửu Vĩ", title: "Thiên Hồ Thần Bí", desc: "Linh hồ ngàn năm ẩn mình nơi lõi rừng già sương phủ, xảo quyệt vô biên, nắm giữ cấm thuật huyễn hoặc thao túng tâm trí lữ khách.", icon: <Feather size={22} />, color: "from-[#FF1493]/15 to-transparent", glow: "shadow-[0_0_30px_rgba(255,20,147,0.15)]" },
    { name: "Hổ Thần", title: "Kim Miêu Trấn Sơn", desc: "Chúa tể sơn lâm vằn vện ánh hoàng kim, mỗi tiếng gầm làm rung chuyển đại ngàn viễn cổ, gánh vác sứ mệnh trấn giữ phong ấn thượng cổ.", icon: <Mountain size={22} />, color: "from-[#FFD700]/15 to-transparent", glow: "shadow-[0_0_30px_rgba(255,215,0,0.15)]" },
    { name: "Xà Linh", title: "Đại Ngàn Thần Xà", desc: "Hắc xà vạn năm cuộn mình dưới mạch địa long cổ thụ, mang nọc độc hủy thiên diệt địa nhưng sở hữu sự thông thái từ thủa sơ khai.", icon: <Map size={22} />, color: "from-[#00E676]/20 to-transparent", glow: "shadow-[0_0_30px_rgba(0,230,118,0.2)]" },
  ];

  // Khai báo kiểu Variants chặt chẽ bảo vệ hệ thống khỏi lỗi TypeScript
  ...
  return (
    <main className="relative min-h-screen bg-[#021812] text-[#E8E0D0] overflow-x-hidden selection:bg-[#00E676]/20 selection:text-[#00E676]">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        ...
        {[...Array(15)].map((_, i) => (
          <motion.div
            ...
            initial={{ y: "110vh", x: `${Math.random() * 100}vw`, scale: Math.random() * 0.8 + 0.4 }}
            animate={{ y: "-10vh", x: [`${Math.random() * 100}vw`, `${Math.random() * 90}vw`] }}
            transition={{ duration: Math.random() * 20 + 15, repeat: Infinity, ease: "linear" }}
            style={{ width: Math.random() * 5 + 3, height: Math.random() * 5 + 3 }}
          />
        ))}
      </div>
      ...
      <section className="relative z-10 flex min-h-screen flex-col items-center justify-center px-6">
        ...
        <motion.div style={{ y: textY }} className="text-center space-y-6 max-w-4xl">
        ...
        <div className="absolute bottom-16 flex flex-col items-center gap-2 opacity-40">
          <Compass size={18} className="animate-[spin_40s_linear_infinite] text-[#00E676]" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          ...
          <div className="lg:col-span-5 space-y-4">
            <div className="mb-10 space-y-2">
              <span className="text-[10px] tracking-[0.3em] text-[#00BFFF] uppercase block font-semibold">Địa long dị tộc</span>
              <h2 className="font-[family-name:var(--font-cormorant)] text-4xl md:text-5xl text-[#E8E0D0] tracking-wide">Bốn Đại Linh Thú</h2>
            </div>
              ...
              {mythicalCreatures.map((creature, idx) => (
                ...
                >
                  <div className="flex items-center gap-6 relative z-10">
                    <span className={`text-xs font-mono transition-colors duration-300 ${activeBeast === idx ? "text-[#00E676]" : "text-[#E8E0D0]/30"}`}>
                      0{idx + 1}
                    </span>
                    <div>
                      <h3 className={`font-[family-name:var(--font-cormorant)] text-2xl transition-colors duration-300 ${activeBeast === idx ? "text-[#00E676]" : "text-[#E8E0D0]/70 group-hover:text-white"}`}>
                        {creature.name}
                      </h3>
                  ...
                  <div className={`transition-all duration-500 ${activeBeast === idx ? "text-[#00E676] scale-110" : "text-[#E8E0D0]/20 group-hover:text-[#00E676]/50"}`}>
                    {creature.icon}
                  </div>
                </button>
              ))}
          ...
          <div className="lg:col-span-7 h-[420px] bg-[#01120e]/60 border border-[#00E676]/10 p-12 relative overflow-hidden flex flex-col justify-between rounded-sm backdrop-blur-sm">
            <div className={`absolute inset-0 bg-gradient-to-br ${mythicalCreatures[activeBeast].color} opacity-40 transition-all duration-700`} />
            <div className={`absolute -top-20 -right-20 w-48 h-48 rounded-full blur-[80px] bg-current opacity-10 transition-all duration-700 ${mythicalCreatures[activeBeast].glow}`} />
      ...
      <section className="relative z-10 py-40 px-6 max-w-7xl mx-auto border-t border-[#00E676]/5 bg-gradient-to-b from-transparent via-[#01140f]/40 to-transparent">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-20">
          ...
          <motion.div 
            variants={textVariant} initial="hidden" whileInView="visible" viewport={{ once: true }}
            className="space-y-6 md:p-8"
          >
            <span className="text-[#00BFFF] text-[10px] tracking-[0.5em] uppercase font-bold block">Khởi nguồn tổ tiên</span>
            <h2 className="font-[family-name:var(--font-cormorant)] text-4xl text-[#E8E0D0] tracking-wider">Linh Sơn & Cổ Tự</h2>
            <p className="text-sm text-[#E8E0D0]/50 leading-[2.2] font-light tracking-wide">
              Những ngọn núi khổng lồ đột ngột nhô lên tột cùng ranh giới từ những tầng mây độc mờ mịt, nối liền mạch sống giữa trời và đất. Nơi đỉnh non cao rêu phong phủ lối là các cổ tự hoang tàn rùng lạnh, địa điểm chôn giấu những lớp thạch trận phức tạp từ thuở hồng hoang nhằm kìm hãm hung lực cổ xưa trỗi dậy.
            </p>
            <div className="pt-4 grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs text-[#00E676]/80 font-mono tracking-widest">
              <div className="flex items-center gap-2">✦ Chinh phục Linh Sơn</div>
              <div className="flex items-center gap-2">✦ Giải mã trận Cổ Tự</div>
              <div className="flex items-center gap-2 col-span-2">✦ Gia cố Mạch Phong Ấn Địa Long</div>
            </div>
          </motion.div>
          ...
          >
            <span className="text-[#00E676] text-[10px] tracking-[0.5em] uppercase font-bold block">Vùng địa cực vô danh</span>
            <h2 className="font-[family-name:var(--font-cormorant)] text-4xl text-[#E8E0D0] tracking-wider">Bí Cảnh Lãng Quên</h2>
            <p className="text-sm text-[#E8E0D0]/50 leading-[2.2] font-light tracking-wide">
              Tồn tại cô độc ngoài rìa của những tấm bản đồ lục giới, đó là hệ thống hòn đảo đá treo lơ lửng giữa biển mây mù hoặc thung lũng sâu thẳm đầy chướng khí. Chỉ những kẻ can trường, mang ý chí sắt đá nhất mới dám đặt chân vào để săn lùng mảnh vỡ sức mạnh của thần linh cổ đại.
            </p>
            <div className="h-[1px] w-full bg-gradient-to-r from-[#00E676]/20 to-transparent my-6" />
            <h4 className="text-md font-[family-name:var(--font-cormorant)] text-[#00BFFF] tracking-widest">Kẻ Khai Phá Vô Danh</h4>
            <p className="text-xs text-[#E8E0D0]/40 leading-relaxed italic font-light">
              "Thợ săn dị thú, lữ khách cô độc hành hương, hay những đạo sĩ truy cầu sức mạnh nguyên thủy tối cao. Bọn họ bước vào đây đều hiểu rằng nơi này chính là mộ chôn của những huyền thoại."
            </p>
          </motion.div>

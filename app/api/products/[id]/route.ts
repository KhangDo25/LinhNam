...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (data.priceValue !== undefined && (!Number.isFinite(data.priceValue) || data.priceValue < 0)) return NextResponse.json({ error: 'Giá không được âm' }, { status: 400 });
  if (data.stock !== undefined && (!Number.isInteger(data.stock) || data.stock < -1)) return NextResponse.json({ error: 'Kho không hợp lệ (-1 = vô hạn)' }, { status: 400 });
  await connectDB();
